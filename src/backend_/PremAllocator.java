...
public class PremAllocator {
  ...
  public HashSet<Reg> spillTemp = new HashSet<>();//栈上开的内存
  LinkedList<Inst> newInsts;
  ASMFunction curFunc;
  // interference graph
  public static class Edge {//冲突图
    public Reg u, v;
    ...
    for (var block : func.blocks) {
      ...
      for (int i = block.insts.size() - 1; i >= 0; i--) {
        ...
        for (Reg def : inst.getDef())
          for (var l : live)
            addEdge(def, l);//建边
        live.removeAll(inst.getDef());//恢复原样
  ...
  LinkedHashSet<MvInst> nodeMoves(Reg reg) {//reg相关的move指令 与activeMoves，workListMoves这两个集合的并 的交集
    LinkedHashSet<MvInst> ret = new LinkedHashSet<MvInst>(activeMoves);
  ...
  void workOnFunc(ASMFunction func) {
    ...
    for (var block : func.blocks) {
      ...
      for (Inst inst : block.insts) {
        if (inst instanceof LiInst && ((LiInst) inst).pseudoImm instanceof StackImm)
  ...
  void stepSimplify() {//全部进selectstack
  ...
  void addWorkList(Reg reg) {
    if (!preColored.contains(reg) && !moveRelated(reg) && degree.get(reg) < kCnt) {// low degree and no move related
  ...
  void combine(Reg u, Reg v) {//v并到u去
    ...
    if (degree.get(u) >= kCnt && freezeWorkList.contains(u)) {//不符合freezelist的定义了（u的边是原本u,v边的并）
  ...
  void freezeMoves(Reg reg) {//move有关
    for (var mv : nodeMoves(reg)) {
      ...
      if (nodeMoves(v).size() == 0 && degree.get(v) < kCnt) {//u被移除导致v受影响
  ...
  void stepSelectSpill() {
    ...
    for (Reg reg : spillWorkList)  // 避免选择那种由读取前面已溢出的寄存器产生的、活跃范围很小的寄存器
      if (m == null || reg.spillWeight / degree.get(reg) < m.spillWeight / degree.get(m) && !spillTemp.contains(reg))//选择溢出优先级最低的
        m = reg;
