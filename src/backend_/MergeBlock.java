  ...
  public void workOnFunc(ASMFunction func) {
    ...
    for (int i = 1; i < func.blocks.size(); ++i) {
      ...
      if (block.pred.size() == 1 && block.pred.get(0) == lastBlock
          && lastBlock.insts.get(lastBlock.insts.size() - 1) instanceof JumpInst ) {
            
            JumpInst jumpInst=(JumpInst) lastBlock.insts.get(lastBlock.insts.size() - 1);
            if(jumpInst.toBlock == block){
